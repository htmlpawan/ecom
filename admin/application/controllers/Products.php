...
//------------------------------------------Add product controller-------------------------------	 
	...
	public function validation_product()
	{
		...
		if($editid)
		{
			...
			$run = $this->add_product->update_product($data, $editid, $imgData);
			if($run)
			{
		...
		$data = array(
		  ...
		  $run = $this->add_product->insert_product($data, $imgData);
			if($run)
			{
	...
	{	
       $editid = $this->input->POST('editid');
        if($editid)
		{
			...
			redirect(base_url('products/categorylist'));
			}
			else{
			 $this->session->set_flashdata('messageadd','Category error.'); 
			  redirect(base_url('products/categorylist'));
			}
		}else{  	
		$image = $this->do_uploadimage();
		 $data = array(
		  ...
		  $run = $this->add_product->insert_news($data);
			if($run)
			{
			$this->session->set_flashdata('messageadd','Category Added Successfully.'); 
			redirect(base_url('products/categorylist'));
			}
			else{
			 $this->session->set_flashdata('messageadd','Category error.'); 
...
//------------------------------New websit
   public function orderlist()
	{
		$data['alldata'] = $this->add_product->select_order();
		// print_r($data['alldata']);
		// exit();
		$this->load->view('order-list', $data);
    }

	//-------------------for api use in website

	public function productsList()
	{
	  $res = $this->add_product->select_product();

	  echo json_encode($res, true);
	}

	public function categoresList()
	{
	  $res = $this->add_product->news_list();
